...
typedef struct PACKED_STRUCT s_bmp_hdr {
    ...
    u16 reserved1;      /* resererved field typically set to 0 */
    u16 reserved2;
...
typedef struct s_bmp_info_hdr {
    ...
    u32 importantcolours;           /* number of important colours, 0 means all are important */
} bmp_info_hdr;
...
FT_INLINE u8 *parse_bmp_file(char* path, int *width, int *height, int *type) {
    u64 file_size = 0;
